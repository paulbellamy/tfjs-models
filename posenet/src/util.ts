...
export function getBoundingBox(
  ...
  return keypoints.reduce(
    ({ maxX, maxY, minX, minY }, { position: { x, y } }) => {
      return {
...
): Promise<tf.TensorBuffer<rank>> {
  const tensorData = await tensor.data();

  return tf.buffer(
    tensor.shape,
    type,
    tensorData as Float32Array
  ) as tf.TensorBuffer<rank>;
}

export async function toTensorBuffers3D(
  tensors: tf.Tensor3D[]
...
export function getInputTensorDimensions(
  input: PosenetInput
): [number, number] {
...
export function padAndResizeTo(
  ...
  return {
    resized,
    padding: { top: padT, left: padL, right: padR, bottom: padB }
  };
}
